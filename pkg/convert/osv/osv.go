...
// Convert converts OSV JSON to Grafeas Note/Occurrence format.
func Convert(ctx context.Context, s *src.Source) (map[string]types.NoteOccurrences, error) {
	...
	for _, r := range s.Data.Search("results").Children() {
		for _, p := range r.Search("packages").Children() {
			for _, v := range p.Search("vulnerabilities").Children() {
				for _, a := range v.Search("aliases").Children() {
					cve := a.Data().(string)
